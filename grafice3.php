...
<div id="selectBox" class="selectBox">
  <select id="statistici_doze" class="statistici_doze">
...
for(var i=0;i<Vaccine.length;i++){
  ...
  var john5 = john+","+john2+","+john3+","+john4;
  console.log(john5);
  //console.log(john);
  //var judet = escape(JSON.stringify((Vaccine[i].Nume)));
  var judet = Vaccine[i].Nume;
  var x = document.getElementById("statistici_doze");
...
const myChart = new Chart(ctx, {
  type: 'doughnut',
  data: {
    labels: ['Doze Pfizer','Doze Moderna','Doze Astra Zeneca','Johnson & Johnson'],
    datasets:[{
      label: '# of Votes',
      data: [785,256,556,634],
      backgroundColor:[
                '#2996ff',
                '#d633ad',
                '#5cd65c',
  ...
  const statistici_doze = document.getElementById('statistici_doze');
  statistici_doze.addEventListener('change', Grafice);
  function Grafice(){
    console.log(statistici_doze.value);
    myChart.data.datasets[0].data = statistici_doze.value.split(',');
    myChart.update();
  }
</script>
</body>
